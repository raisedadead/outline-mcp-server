...
export function registerTools(server, client) {
    ...
    server.tool('outline_search', 'Search documents in Outline wiki by query', {
    ...
    server.tool('outline_get_document', 'Get a document by ID or URL ID, returns full markdown content', {
    ...
    server.tool('outline_list_collections', 'List all collections in the Outline workspace', {}, async () => {
    ...
    server.tool('outline_list_documents', 'List documents, optionally filtered by collection', {
        collectionId: z.string().optional().describe('Filter by collection ID'),
    ...
    server.tool('outline_create_document', 'Create a new document in Outline', {
    ...
    server.tool('outline_update_document', 'Update an existing document in Outline', {
    ...
    server.tool('outline_move_document', 'Move a document to a different collection or parent document', {
    ...
    server.tool('outline_delete_document', 'Delete a document from Outline', {
    ...
    server.tool('outline_archive_document', 'Archive a document (soft delete, can be restored)', {
        ...
        return {
            content: [
                {
                    type: 'text',
                    text: JSON.stringify({ archived: true, id: doc.id, title: doc.title, archivedAt: doc.archivedAt }, null, 2)
                }
            ]
        };
    });
    // outline_unarchive_document - Restore an archived document
    server.tool('outline_unarchive_document', 'Restore an archived document', {
    ...
    server.tool('outline_list_drafts', 'List all draft (unpublished) documents', {}, async () => {
    ...
    server.tool('outline_export_document', 'Export a document as clean markdown', {
    ...
    server.tool('outline_get_collection', 'Get details of a specific collection', {
    ...
    server.tool('outline_create_collection', 'Create a new collection in Outline', {
    ...
    server.tool('outline_update_collection', 'Update an existing collection', {
    ...
    server.tool('outline_delete_collection', 'Delete a collection and all its documents', {
